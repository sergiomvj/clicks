...
- contratos publicos de `1FBR-Leads`, `1FBR-Dev` e `1FBR-Suporte` respondem `accepted`\r\n- `1FBR-Dev` e `1FBR-Suporte` agora tem callback de saida preparado no backend, pendente apenas da URL real
...
- mensagens, tarefas e `agent-api` agora tambem possuem limite adicional por canal e por agente quando aplicavel
...
- `agent-api` ja cria tarefas reais de follow-up e tarefas de revisao de stage
- `agent-api` ja registra mensagem de contexto no canal ao sugerir mudanca de stage
- `agent-api` ja registra rascunho real de mensagem no canal
- `agent-api` ja executa `change_deal_stage` apos approval aprovado
...
- a tela do canal agora tem bridge realtime e atualiza via `router.refresh()` sem refresh manual
- as telas de `tasks`, `pipeline` e `agents` agora atualizam via websocket de workspace sem refresh manual
